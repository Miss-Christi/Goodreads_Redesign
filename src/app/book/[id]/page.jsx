...
import { Star, ArrowLeft, BookOpen, Share2, ThumbsUp, MessageSquare } from 'lucide-react';
import BookCover from '@/components/BookCover'; // Importing the component we just made
             ...
             <div className="lg:col-span-3 flex flex-col gap-6">
               ...
               <div className="flex flex-col gap-3">
                 ...
                 <div className="flex gap-2">
                    <button className="flex-1 bg-white/5 hover:bg-white/10 text-white font-medium py-3 rounded-lg border border-white/10 transition-colors">Preview</button>
                    <button className="bg-white/5 hover:bg-white/10 text-white font-medium py-3 px-4 rounded-lg border border-white/10 transition-colors"><Share2 className="w-5 h-5" /></button>
                 </div>
               </div>
             </div>
                        ...
                        {recommendations.map(rb => (
                           <Link key={rb.id} href={`/book/${rb.id}`} className="group block bg-primary-dark rounded-xl p-3 hover:bg-white/5 transition-all border border-transparent hover:border-white/10">
                              <div className="relative aspect-[2/3] rounded-lg overflow-hidden mb-3 shadow-lg group-hover:scale-105 transition-transform">
                                 {/* 3. Recommended Cover */}
                                 <BookCover src={rb.CoverUrl} alt={rb.Title} />
                              </div>
